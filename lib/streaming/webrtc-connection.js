...
export class WebrtcConnection extends Subscribed {
    ...
    // Un-mute the camera speaker so return (talk-back) audio is actually played by
    // the device. Without camera_options{stealth_mode:false} Ring keeps the speaker
    // muted and two-way audio is silent (ref: ring-client-api activateCameraSpeaker).
    // Idempotent; fire-and-forget once the camera is connected.
    ...
    // Re-export of the peer-connection send path (mirrors onAudioRtp/onVideoRtp) so the
    // StreamingSession can push return audio without reaching into nested .pc internals.
    sendAudioRtp(rtp) {
        if (this.hasEnded) {
            return
        }
        this.pc.sendAudioRtp(rtp)
    }
}
